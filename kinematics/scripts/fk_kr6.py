...
T55p=dh_matrix(t5-pi/2,0.0,0.08,0)
T5p6=dh_matrix(-pi/2,0.0,0.0,pi/2)
T67=dh_matrix(t6,0.0,0.0,0.0)
...
def callback(data):
    ...
    T07n=T06.subs([(t1,data.linear.x),(t2,data.linear.y),(t3,data.linear.z),(t4,data.angular.x),(t5,data.angular.y),(t6,data.angular.z)])
    position.x= T07n[0,3]
    position.y= T07n[1,3]
    position.z= T07n[2,3]
